...
contract SushiPetsMarket {
    ...
    function setThreshold(uint _tokenId, uint _value, bool _active) external {
        require(sushiPetsToken.totalPetsMinted() <= _tokenId, "ERC721: token has not been minted.");
        require(msg.sender == sushiPetsToken.ownerOf(_tokenId), "Only the token owner can accept the bid.");
    ...
    function makeBid(uint _tokenId, uint _bidValue) external payable {
        ...
        if(threshold[_tokenId].active && msg.value >= threshold[_tokenId].value) {
            ...
            (bool sent,) = sushiPetsToken.ownerOf(_tokenId).call{value: msg.value}("");
            ...
            emit BidAccepted(_tokenId, sushiPetsToken.ownerOf(_tokenId), msg.sender, bid.bidValue);
        } else {
    ...
    function acceptBid(uint _tokenId) external payable {
        ...
        sushiPetsToken.safeTransferFrom(msg.sender, bid.bidder, _tokenId, "");
    ...
    function executeBid(uint _tokenId, address _bidder) internal {
